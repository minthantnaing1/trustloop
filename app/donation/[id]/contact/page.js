...
export default function ContactDonorPage() {
      ...
      try {
        ...
        if (meRes.ok) {
          ...
          if (!message) {
            ...
            setMessage(`Hi ${firstName}! I'm interested in the ${p?.title || "item"}. When would be a good time to pick it up?`);
          }
        }
      } catch (e) {
        // ignore
  ...
  async function onSubmit(e) {
    ...
    // Optional AU email guard
    // const auEmail = /@(au\.edu|university\.edu)$/i;
    // if (!auEmail.test(email)) return alert("Please use your AU student email.");
    ...
    try {
      ...
      router.push("/donation/requests?tab=sent");
    } catch {
  ...
  const isFree = product?.type === "donation" || Number(product?.price) === 0;

  return (
    <>
      <NavBar />
      <main className="max-w-[1200px] mx-auto mt-[120px] mb-[40px] px-5 w-full">
        <Link href={`/donation/${product._id}`} className="text-[#325082] text-sm hover:underline flex items-center gap-1 mb-4">
