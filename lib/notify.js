...
function linkFor({ type, flags, txn }) {
  ...
  const kind =
    k === "DONATION" || pt === "donation"
      ? "DONATION"
      : k === "AUCTION" || pt === "auction"
        ? "AUCTION"
        : "BUY_SELL";
  ...
  // Buyer/Recipient (also covers AUCTION bidder/winner side)
  if (isBuyer) {
    // For BUY_SELL and AUCTION, after completion go to review
    if (
      type === "ADMIN_PAID_OUT" &&
      (kind === "BUY_SELL" || kind === "AUCTION")
    )
  ...
  // Seller/Donor (also covers AUCTION seller side)
  if (isSeller) {
    // For BUY_SELL and AUCTION, payout page after admin payout
    if (
      type === "ADMIN_PAID_OUT" &&
      (kind === "BUY_SELL" || kind === "AUCTION")
    )
    ...
    // Donation uses payout/summary after confirmation
    if (type === "BUYER_CONFIRMED" || type === "AUTO_CONFIRMED_AFTER_3_DAYS")
      return `/my-orders/${txn._id}/payout`;
...
function textFor({ type, txn, flags }) {
  ...
  /* ---------- AUCTION (minimal: reuse Buy/Sell events, change wording) ---------- */
  const isAuction =
    String(txn?.kind || "").toUpperCase() === "AUCTION" ||
    ...
    switch (upType) {
      case "ORDER_CREATED":
        if (isBuyer)
          return {
            title: "Auction won",
            message: `You won the auction for “${product}”. Complete payment within the time limit.`,
          };
        if (isSeller)
          return {
            title: "Auction has a winner",
            message: `Your auction “${product}” has a winner. Waiting for payment.`,
          };
        if (isAdmin)
          return {
            title: "Auction transaction created",
            message: `An auction transaction was created for “${product}”.`,
          };
        break;
      ...
      case "STRIPE_PAYMENT_CONFIRMED":
        if (isBuyer)
          return {
            ...
            message: `Stripe payment successful for auction “${product}”.`,
          };
        if (isSeller)
          return {
            title: "Payment received",
            message: `Winner paid for “${product}”. You can proceed with delivery/meetup.`,
          };
        if (isAdmin)
          return {
            title: "Payment successful",
      ...
      case "AUTO_CANCELLED_EXPIRED":
        if (isBuyer)
          return {
            ...
            message: `Payment time expired for auction “${product}”. The win was cancelled.`,
          };
        if (isSeller)
          return {
            title: "Winner payment expired",
            message: `Winner didn't pay in time for “${product}”. You may need to offer it to the next bidder (if your system supports it).`,
          };
        if (isAdmin)
          return {
            title: "Auction payment expired",
            message: `Payment time expired for auction “${product}”.`,
          };
        break;

      // For all other statuses/events, reuse your BUY_SELL messages (no extra work)
      default:
        break;
    }
  }
