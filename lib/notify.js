...
function shouldEmailRecipient({ type, txn, flags }) {
  ...
  if (isAdmin && !isBuyer && !isSeller) {
    return (
      ...
      (upType !== "CANCELLED_BY_BUYER" && upType !== "CANCELLED_BY_SELLER"
        ? true
        : paidFlow && txn?.hasPaymentSucceeded)
    );
  }

  // Buyer / Seller important emails
  if (isBuyer || isSeller) {
    return [
      "ORDER_CREATED",
      "STRIPE_PAYMENT_CONFIRMED",
      ...
      case "STRIPE_PAYMENT_CONFIRMED":
        if (isBuyer) {
          return {
            ...
            message: `Your payment for auction “${product}” was successful.`,
          };
        }
        if (isSeller) {
          return {
      ...
      case "AUTO_CANCELLED_EXPIRED":
        ...
        if (isSeller) {
          return {
            ...
            message: `The winner did not pay in time for “${product}”. The auction may move to the next bidder.`,
          };
        }
        return null;
      ...
      case "ADMIN_PAID_OUT":
        if (isBuyer) {
          return {
            ...
            message: `Payout was released for “${product}”. Please leave a review if you haven't.`,
          };
        }
        if (isSeller) {
          return {
            title: "Payout released",
            message: `Admin released your payout for “${product}”.`,
          };
        }
        if (isAdmin) {
          return {
            title: "Payout released",
            message: `Payout released for auction “${product}”.`,
          };
        }
        return null;
